...
class ConversationService {
  ...
  String getConversationId(String uuid) {
  ...
  // 실시간 메시지 스트림 가져오기
  Stream<QuerySnapshot> getMessagesStream(String uuid) {
    final conversationId = getConversationId(uuid);
    return _firestore
  ...
  // 메시지 전송
  Future<void> sendMessage(String uuid, String text, String sender) async {
    final conversationId = getConversationId(uuid);
    final uid = _auth.currentUser!.uid;
  ...
  Future<Map<String, dynamic>> getConversationContext(String uuid) async {
    final conversationId = getConversationId(uuid);
    final conversationRef =
        _firestore.collection('conversations').doc(conversationId);
